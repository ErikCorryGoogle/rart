...
#define FATAL(error) \
  rart::Assert(__FILE__, __LINE__).Fail("%s", error)

#define FATAL1(format, p1) \
  rart::Assert(__FILE__, __LINE__).Fail(format, (p1))
...
#define ASSERT(condition) \
  if (!(condition)) { \
    rart::Assert(__FILE__, __LINE__).Fail("expected: %s", #condition); \
  }
...
// argument.
#define ASSERT(condition) do { while (false && (condition)) {} } while(false)
...
#define ASSERT_EQ(expected, actual) \
  rart::Assert(__FILE__, __LINE__).Equals((expected), (actual))

#define ASSERT_STREQ(expected, actual) \
  rart::Assert(__FILE__, __LINE__).StringEquals((expected), (actual))

#define ASSERT_LT(left, right) \
  rart::Assert(__FILE__, __LINE__).LessThan((left), (right))

#define ASSERT_LE(left, right) \
  rart::Assert(__FILE__, __LINE__).LessEqual((left), (right))

#define ASSERT_GT(left, right) \
  rart::Assert(__FILE__, __LINE__).GreaterThan((left), (right))

#define ASSERT_GE(left, right) \
  rart::Assert(__FILE__, __LINE__).GreaterEqual((left), (right))

#define EXPECT(condition) \
  if (!(condition)) { \
    rart::Expect(__FILE__, __LINE__).Fail("expected: %s", #condition); \
  }

#define EXPECT_EQ(expected, actual) \
  rart::Expect(__FILE__, __LINE__).Equals((expected), (actual))

#define EXPECT_STREQ(expected, actual) \
  rart::Expect(__FILE__, __LINE__).StringEquals((expected), (actual))

#define EXPECT_LT(left, right) \
  rart::Expect(__FILE__, __LINE__).LessThan((left), (right))

#define EXPECT_LE(left, right) \
  rart::Expect(__FILE__, __LINE__).LessEqual((left), (right))

#define EXPECT_GT(left, right) \
  rart::Expect(__FILE__, __LINE__).GreaterThan((left), (right))

#define EXPECT_GE(left, right) \
  rart::Expect(__FILE__, __LINE__).GreaterEqual((left), (right))
