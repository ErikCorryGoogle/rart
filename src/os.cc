...
i64 OS::CurrentTime() {
  ...
  return (static_cast<i64>(tv.tv_sec) * 1000000) + tv.tv_usec;
}
...
char* OS::LoadFile(const char* uri, Zone* zone, u32* file_size) {
  // Open the file.
...
bool OS::StoreFile(const char* uri, List<u8> bytes) {
  // Open the file.
