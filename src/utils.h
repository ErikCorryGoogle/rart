...
class Utils {
 public:
  ...
  static u32 StringHash(const uint16_t* data, int length);
  ...
  static bool SignedMulOverflow(word lhs, word rhs, word* val) {
    ...
    word res = lhs * rhs;
    bool overflow = (res != static_cast<i64>(lhs) * rhs);
  ...
  static inline i32 ReadInt32(u8* buffer) {
    return reinterpret_cast<i32*>(buffer)[0];
  }

  // Write a 32-bit integer to the buffer, as little endian.
  static inline void WriteInt32(u8* buffer, i32 value) {
    reinterpret_cast<i32*>(buffer)[0] = value;
  }

  // Read a 64-bit integer from the buffer, as little endian.
  static inline i64 ReadInt64(u8* buffer) {
    return reinterpret_cast<i64*>(buffer)[0];
  }

  // Write a 64-bit integer to the buffer, as little endian.
  static inline void WriteInt64(u8* buffer, i64 value) {
    reinterpret_cast<i64*>(buffer)[0] = value;
  }
};
